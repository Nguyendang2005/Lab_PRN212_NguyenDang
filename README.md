...
Repository hiện tại bao gồm **6 thư mục chính** tương ứng với các bài học và bài tập thực hành khác nhau trên lớp:
...
| 5 | **[DemoWindowPage](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage)** | WPF Windows & Pages | Minh họa thiết kế giao diện bằng XAML, sử dụng các Panel và Page Navigation. |
| 6 | **[Democode_DB](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/Democode_DB)** | `Entity Framework Core` - Code First & Database First | Ứng dụng Windows Forms quản lý Categories sử dụng EF Core với approach Code First. |
...
### 📂 5. DemoWindowPage
*Đường dẫn mã nguồn chính:* [MainWindow.xaml.cs](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/MainWindow.xaml.cs) | [MainWindow.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/MainWindow.xaml)

#### 📝 Giới thiệu & Mục tiêu
Project minh họa giao diện đồ họa **WPF (Windows Presentation Foundation)**. Dự án tập trung vào cách thiết kế giao diện bằng XAML, cách sử dụng các loại Layout Panel khác nhau và cơ chế điều hướng (Navigation) giữa các Page trong ứng dụng WPF.

#### ⚙️ Các khái niệm & Kiến thức áp dụng
*   **WPF Layout Panels**:
    *   **Grid Panel ([GridPanel.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/GridPanel.xaml))**: Sắp xếp giao diện theo các dòng (Row) và cột (Column) linh hoạt.
    *   **Dock Panel ([DockPanel.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/DockPanel.xaml))**: Đính các phần tử UI vào các cạnh (Top, Bottom, Left, Right) của màn hình.
    *   **Wrap Panel ([WrapPanel.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/WrapPanel.xaml))**: Tự động xuống dòng khi các phần tử vượt quá chiều rộng của container.
    *   **Stack Panel ([StackPanel.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/StackPanel.xaml))**: Xếp chồng các phần tử UI theo chiều dọc (Vertical) hoặc chiều ngang (Horizontal).
    *   **Canvas ([Canvas.xaml](file:///d:/Hoc_web_C%23/Lab_PRN212_NguyenDang/DemoWindowPage/DemoWindowPage/Canvas.xaml))**: Đặt các phần tử tại các vị trí tọa độ tuyệt đối xác định bằng `Canvas.Left`, `Canvas.Top`.
*   **Page Navigation**: Sử dụng đối tượng `Frame` (`MainFrame`) để quản lý việc chuyển đổi qua lại giữa các trang `Page1`, `Page2`, và các Panel Pages thông qua phương thức `Navigate()`.

---

### 📂 6. Democode_DB
*Đường dẫn mã nguồn chính:* [MyStockDBContext.cs](file:///Democode_DB/Democode_DB/MyStockDBContext.cs) | [frmManageCategories.cs](file:///Democode_DB/Democode_DB/frmManageCategories.cs) | [ManageCategories.cs](file:///Democode_DB/Democode_DB/ManageCategories.cs)

#### 📝 Giới thiệu & Mục tiêu
Project này minh họa cách sử dụng **Entity Framework Core** để làm việc với cơ sở dữ liệu SQL Server thông qua cả hai phương pháp:
- **Code First**: Định nghĩa Entity Class trước, sau đó tự động tạo database và bảng
- **Database First**: Kết nối với database có sẵn và thực hiện các thao tác CRUD

Ứng dụng Windows Forms cho phép quản lý danh mục sản phẩm (Categories) với đầy đủ chức năng Insert, Update, Delete, và Read.

#### ⚙️ Các khái niệm & Kiến thức áp dụng
*   **Entity Framework Core**: Framework ORM (Object-Relational Mapping) hiện đại của .NET để tương tác với database.
*   **DbContext (`MyStock`)**: Lớp kế thừa từ `DbContext` đại diện cho phiên làm việc với database, chứa `DbSet<Category>` đại diện cho bảng Categories.
*   **Entity Class (`Category`)**: 
    *   Sử dụng Data Annotations: `[Key]` đánh dấu khóa chính, `[DatabaseGenerated]` xác định auto-increment.
    *   Thuộc tính `CategoryID` và `CategoryName` ánh xạ trực tiếp với các cột trong database.
*   **Configuration Management**: 
    *   Đọc connection string từ file `appsettings.json` sử dụng `IConfigurationRoot`.
    *   Connection string: `Server=DANG;uid=sa;pwd=123456;database=MyStockDB;TrustServerCertificate=True`
*   **Fluent API**: Sử dụng trong `OnModelCreating()` để cấu hình ràng buộc cho `CategoryName` (required, max length 40 ký tự).
*   **Repository Pattern (`ManageCategories`)**: 
    *   Tách biệt logic truy xuất dữ liệu khỏi UI.
    *   Các phương thức CRUD: `InsertCategory()`, `UpdateCategory()`, `DeleteCategory()`, `GetCategories()`, `GetCategoryByID()`.
    *   Sử dụng `EntityState.Modified` để đánh dấu entity cần update.
*   **Windows Forms UI**:
    *   DataGridView hiển thị danh sách categories.
    *   TextBox để nhập/chỉnh sửa thông tin.
    *   Các nút Insert, Update, Delete để thực hiện các thao tác CRUD.
    *   Input validation và xác nhận trước khi xóa.
*   **Migration**: Folder `Migrations/` chứa các file migration được tạo tự động bởi EF Core Tools để tạo/cập nhật schema database.

#### 🔧 Cấu hình & Yêu cầu
*   **.NET 8.0** với Windows Forms enabled
*   **SQL Server** đang chạy với thông tin kết nối trong `appsettings.json`
*   **Các NuGet Packages**:
...
#### 🚀 Hướng dẫn chạy
1. Đảm bảo SQL Server đang chạy và thông tin kết nối trong `appsettings.json` đúng
2. Mở Package Manager Console và chạy migration:
   ```powershell
   Update-Database
   ```
3. Build và chạy ứng dụng từ Visual Studio hoặc:
   ```bash
   dotnet run
   ```
